...
export default function LeadDetailPage({ params }: Props) {
  ...
  const returnTo = searchParams.get("returnTo") === "/crm/pipeline" ? "/crm/pipeline" : "/crm/leads";
    ...
    <main className="min-h-screen bg-slate-50">
      <header className="bg-white border-b border-slate-200 px-6 py-4 flex items-center gap-3">
        ...
        <p className="text-xs text-slate-500">
          <Link href={returnTo} className="hover:text-indigo-600">{returnTo === "/crm/pipeline" ? "Pipeline" : "Leads"}</Link>
          {" / "}
