...
import {
  ...
  Users as UsersIcon, UserCheck, UserX, ShieldCheck, AlertCircle, X, Eye, EyeOff, ExternalLink,
} from "lucide-react";
import { usersApi, type User } from "@/lib/api/users";
import { backendRolesApi, type BackendRole } from "@/lib/api/roles";
...
function RoleBadge({ role, code }: { role: string; code?: string }) {
  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold ${ROLE_COLORS[code ?? role] ?? "bg-slate-100 text-slate-600"}`}>
      {role}
    </span>
  );
}
...
function ActionButtons({
  ...
  return (
    <div className="flex items-center justify-center gap-0.5">
      <Link href={`/settings/users-access/users/${user.id}`} title="View Access & Permissions" className={`${btn} text-slate-600 hover:bg-slate-100`}>
...
function UserFormModal({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!form.roleId) return setLocalError("Role is required");
...
export default function UsersPage() {
  ...
  const fetchUsers = useCallback(async () => {
    ...
    setError(null);
    try {
      const [data, backendRoles] = await Promise.all([
        usersApi.list(),
        backendRolesApi.list(),
      ]);
      setUsers(data ?? []);
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">Manage user accounts and their assigned role</p>
        </div>
        <button
                  ...
                  displayedUsers.map((user) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-center">
                        <RoleBadge role={user.assignedRole?.name ?? user.role} code={user.assignedRole?.code ?? user.role} />
                      </td>
                      <td className="px-4 py-3 text-center">
      ...
      {showCreateModal && (
        <UserFormModal
          mode="create"
          initial={{ name: "", email: "", password: "", roleId: roles.find((role) => role.code === "STAFF")?.id ?? roles[0]?.id ?? "", isActive: true }}
