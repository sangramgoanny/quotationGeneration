...
export function apiToRecord(q: Quotation): QuotationRecord {
...
async function buildQuotationPdf(q: QuotationRecord) {
  ...
  const addBg = () => {
    if (hasLH) doc.addImage(img, "JPEG", 0, 0, 210, 297, "quotation-letterhead", "FAST");
  };
  ...
  if (hasStamp) {
    doc.addImage(stamp, "PNG", stampDrawX, y, stampMmW, stampMmH, "quotation-stamp", "FAST");
  } else {
    sd(C.border); doc.setLineWidth(0.3);
...
async function downloadAgreementPDF(q: QuotationRecord) {
  ...
  const hasLH = img.complete && img.naturalWidth > 0;
  const addBg = () => {
    if (hasLH) doc.addImage(img, "JPEG", 0, 0, 210, 297, "agreement-letterhead", "FAST");
  };
  ...
  const stampCentreX = R - stampMmW / 2;
  if (hasStamp) {
    doc.addImage(stamp, "PNG", stampDrawX, sigBaseY, stampMmW, stampMmH, "agreement-stamp", "FAST");
  } else {
    sd(C.border); doc.setLineWidth(0.3);
